...
class KTH(DatasetBase):
    def __init__(self, size, type, length=None, keys=None):
        super().__init__()
        ...
        with open(f"./data/KTH/{type}-mini.txt", "r") as f:
            lines = f.read().splitlines()

        # it means a line should have filename, start_idx and end_idx
        # like 'walking/person18_walking_d4_uncomp.avi 1 80'
        if length is None:
            assert len(lines[0].split()) == 3, "dataset should be [filepath, start_idx, end_idx]"
        ...
        # like 'walking/person18_walking_d4_uncomp.avi'
            ...
            start_idxs = [0]*len(lines)
            end_idxs = [length]*len(lines)
