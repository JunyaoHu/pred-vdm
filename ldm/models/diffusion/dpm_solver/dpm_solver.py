...
class DPM_Solver:
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        method='multistep', lower_order_final=True, denoise_to_zero=False, solver_type='dpmsolver',
        atol=0.0078, rtol=0.05, return_intermediate=False,log_every_t=5,
    ):
        """
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                if return_intermediate:
                    rank_zero_info(f"DPM solver multistep {step} {log_every_t}")
                ...
                for step in range(order, steps + 1):
                    ...
                    if self.correcting_xt_fn is not None:
                        x = self.correcting_xt_fn(x, t, step)
                    if return_intermediate:
                        if step % log_every_t == 0 or step == steps:
                            rank_zero_info(f"DPM solver multistep {step} {log_every_t}")
                            intermediates.append(x)
                ...
                for step, order in enumerate(orders):
                    ...
                    if self.correcting_xt_fn is not None:
                        x = self.correcting_xt_fn(x, t, step)
                    if return_intermediate:
                        rank_zero_info(f"DPM solver singlestep {step} {log_every_t}")
                        intermediates.append(x)
            else:
            ...
            if denoise_to_zero:
                ...
                if return_intermediate:
                    rank_zero_info(f"DPM solver denoise_to_zero. ")
                    intermediates.append(x)
        if return_intermediate:
